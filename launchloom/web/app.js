...
function renderRevise(){
  ...
  return `<details class="panel revise-panel"><summary>構成を直して、この素材のまま作り直す${c.revision?` · 改訂 ${c.revision}`:''}</summary><p class="notice">収録済みの映像をそのまま使います。再収録も、生成AIへの再依頼も行いません。作り直すと現在の動画・LP・キットは置き換わります。</p>${storyboardForm(plan,c.options)}<div class="dialog-actions"><span>投稿済みの内容は変わりません。</span><button class="button dark" id="revise-plan">この内容で作り直す ↻</button></div><p class="error" id="revise-error"></p></details>`;
}
function renderFilm(){
...
function renderDistribution(){
  ...
  const pacing=`同じSNSへは${state.config.min_post_gap_minutes||30}分以上あけ、1日${state.config.max_posts_per_channel_per_day||3}件までにしています。`;
...
async function checkRemoteStates(){
  ...
  try{
    ...
    toast(`${r.items.length}件を確認しました。${absent?`${absent}件はPostizに見つかりません。`:''}`);
    await refresh(true);
...
async function previewDeployment(){
  ...
  body.innerHTML=`<p class="deploy-target">書き込み先：<b>${escape(preview.target)}</b></p>${rows}<p class="notice">このディレクトリの他のファイル（${preview.left_untouched.length?escape(preview.left_untouched.slice(0,6).join(', ')):'なし'}${preview.left_untouched.length>6?' ほか':''}）はそのまま残ります。</p><div class="dialog-actions"><span>承認は、いま確認したこの内容に対してのみ有効です。</span><button class="button dark" id="deploy-confirm" ${state.current.released?'':'disabled'}>この内容で書き出す ↗</button></div><p class="error" id="deploy-error"></p>`;
...
function settings(){
  ...
  $('settings-body').innerHTML=`<p>キーはサーバーの .env で設定して再起動します。ブラウザにAPIキーは保存しません。</p>
    <div class="wizard">
      ...
      ${connectionStep(accounts.length>0,'2. 接続を確かめる',accounts.length?`${accounts.length}件の投稿先が見つかりました：${accounts.map(a=>escape(a.name||a.id)).join('、')}`:'「投稿先を読み込む」で、Postizが持っているアカウントを取得します。','<button class="button small" id="wizard-integrations">投稿先を読み込む ↻</button>')}
