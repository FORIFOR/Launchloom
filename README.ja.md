...
通常スタジオの **「シーン・完成動画」** から、選択中のキャンペーンを開けます。
Seedance 2.5の生成依頼はシーンごとに内容・費用を承認。AEなどの完成動画は
MP4/MOV/WebMで取り込み、確認した版を採用して配信画面へ進めます。動画の採用を
変えると古い未送信の承認は無効になり、公開許可の取り直しが必要です。

Codex / Claude CodeでJSX候補を作るローカルコマンドと、確認済みAEプロジェクトを
書き出すコマンドもあります。外部CLI・Adobe・実SNSでの動作は別途確認が必要です。
**[設定・実行コマンド・検証範囲](docs/PRODUCTION_WORKFLOW.md)**。
